RSpec.describe OrderDecorator do
  ...
  describe '#last_status?' do
    ...
    let(:not_last_status_index) do
      Order.aasm.states.map(&:name).excluding(:in_delivery, :delivered, :canceled).count - 2
    end
    let(:last_status_index) { Order.aasm.states.map(&:name).excluding(:in_delivery, :delivered, :canceled).count - 1 }

    it { is_expected.to be_last_status(last_status_index) }

    it { is_expected.not_to be_last_status(not_last_status_index) }
  end
end
