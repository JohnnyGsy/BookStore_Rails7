...
module Checkout
  class OrderAddressUpdateService < BaseService
    ...
    def call
      update_address_address_type(@address_params[:shipping_address]) unless @address_params[:only_billing]
      update_address_address_type(@address_params[:billing_address])
      @order.delivery_step! if success?
    end

    private

    def update_address_address_type(params)
